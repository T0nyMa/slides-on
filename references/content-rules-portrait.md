...
**制作指南**：`references/portrait-user-guide.md` — Design CSS 用法、组件搭配模式、填满画面技巧。
...
Article 页（`type: "article"`）由异构 block 纵向堆叠。判断一页是否填满，不看 block 数量，看**预估总高度是否接近可用空间**。
...
从原文提取有意义的细节，用**不同组件类型**承载（保持视觉节奏变化）：

| 场景 | 推荐补充 |
|------|---------|
| 有核心结论/原则 | 加 badge-para（定性标签 + 一句话） |
| 有关键技术点 | 加 icon-card（emoji + 标题 + 一行说明） |
| 有总结/金句 | 加 quote-bar（收尾强调） |
| 有标签/关键词 | 加 pill-tags（视觉收束） |

**禁止**：用拉伸间距 / 居中对齐 / 放大字号来"撑满"页面。正确做法永远是增加有意义的内容组件。
