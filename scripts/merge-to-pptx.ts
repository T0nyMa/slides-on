/**
 * merge-to-pptx.ts — PNG slides → PPTX wrapper
 *
 * Adapts render-precise output (page_NN.png) to baoyu-slide-deck naming
 * convention (NN-slide-*.png), then delegates to baoyu's merge-to-pptx.
 */
...
function parseArgs(): { dir: string; output?: string } {
  ...
  if (!dir) {
    console.error("Usage: bun merge-to-pptx.ts <png-dir> [--output filename.pptx]");
    process.exit(1);
  }
...
function main() {
  ...
  const outputPath = output || join(absDir, `${dirName}.pptx`);
  ...
  const baoyuScript = resolve(
    import.meta.dir,
    "..", "..", "baoyu-slide-deck", "scripts", "merge-to-pptx.ts"
  );
...
const isMain = import.meta.url === `file://${process.argv[1]}` || process.argv[1]?.endsWith("merge-to-pptx.ts");
if (isMain) {
  main();
}
